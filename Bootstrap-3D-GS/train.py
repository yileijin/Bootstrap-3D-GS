...
from scene.gen_variant_pipe import gen_image_variants, tensor2PILimage
from scene.gen_variant_ldm import gen_sd_variants
import torchvision
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if not viewpoint_stack:
            viewpoint_stack = scene.getTrainCameras().copy()
            viewpoint_indices = list(range(len(viewpoint_stack)))
        pop_view_idx = randint(0, len(viewpoint_indices) - 1)
        viewpoint_cam = viewpoint_stack.pop(pop_view_idx)
        vind = viewpoint_indices.pop(pop_view_idx)
        ...
        if opt.do_expension and (scene.expended_cams is not None):
            ...
            for variant_cam in variant_cams:
                ...
                render_pkg = render(variant_cam, gaussians, pipe, bg, use_trained_exp=dataset.train_test_exp, separate_sh=SPARSE_ADAM_AVAILABLE)
                exp_image, exp_viewspace_point_tensor, exp_visibility_filter, exp_radii = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]

                if variant_cam.alpha_mask is not None:
                    alpha_mask = variant_cam.alpha_mask.cuda()
                    exp_image *= alpha_mask
                  
                # Loss
                sub_Ll1 = l1_loss(exp_image, expended_image)
                # if FUSED_SSIM_AVAILABLE:
                #     sub_ssim_value = fused_ssim(exp_image.unsqueeze(0), expended_image.unsqueeze(0))
                # else:
                #     sub_ssim_value = ssim(exp_image, expended_image)
                add_loss = add_loss + sub_Ll1 # (1.0 - opt.lambda_dssim) * sub_Ll1 + opt.lambda_dssim * (1.0 - sub_ssim_value)
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                # Bootstrap Densification
                if opt.do_expension and (scene.expended_cams is not None):
                    for i in range(len(exp_viewspace_point_tensors)):
                        cur_visi_filter = exp_visibility_filters[i]
                        gaussians.max_radii2D[cur_visi_filter] = torch.max(gaussians.max_radii2D[cur_visi_filter], exp_radiis[i][cur_visi_filter])
                        gaussians.add_densification_stats(exp_viewspace_point_tensors[i], cur_visi_filter)
            ...
            if opt.do_expension:
                ...
                if do_expension:
                    ...
                    for i in range(len(scene.expended_cams)):
                        ...
                        expended_pkg = render(expended_cam, gaussians, pipe, bg, use_trained_exp=dataset.train_test_exp, separate_sh=SPARSE_ADAM_AVAILABLE)
                        ...
                        if opt.save_novel_renderings:
                            gen_path = os.path.join(opt.gen_variant_path, f'{iteration}', 'img2img')
                            os.makedirs(gen_path, exist_ok=True)
                            save_path = os.path.join(gen_path, f"{i}.png")
                            rendered_img.save(save_path)
                    
                    gaussians._to_cpu()
                    denoised_imgs = gen_image_variants(opt, new_scene_renders, iteration)
                    gaussians._to_cuda()
                    # denoised_imgs = gen_sd_variants(opt, iteration, new_scene_renders)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[7_000, 10000, 15000, 18000, 21000, 24000, 27000, 30_000])
