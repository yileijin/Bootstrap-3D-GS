...
class GaussianModel:
    ...
    # For memory saving during diffusion pipeline
    def _to_cpu(self):
        self.org_device = self._anchor.device
        self._anchor.to("cpu")
        self._offset.to("cpu")
        self._anchor_feat.to("cpu")
    ...
    def _to_cuda(self):
        self.org_device = self._anchor.device
        self._anchor.to(self.org_device)
        self._offset.to(self.org_device)
        self._anchor_feat.to(self.org_device)

        self._scaling.to(self.org_device)
        self._rotation.to(self.org_device)
        self._opacity.to(self.org_device)
        self.max_radii2D.to(self.org_device)
    ...
    def load_ply_sparse_gaussian(self, path):
        ...
        opacities = np.asarray(plydata.elements[0]["opacity"])[..., np.newaxis].astype(float)

        scale_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("scale_")]
        scale_names = sorted(scale_names, key = lambda x: int(x.split('_')[-1]))
        scales = np.zeros((anchor.shape[0], len(scale_names)))
        for idx, attr_name in enumerate(scale_names):
            scales[:, idx] = np.asarray(plydata.elements[0][attr_name]).astype(float)

        rot_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("rot")]
        rot_names = sorted(rot_names, key = lambda x: int(x.split('_')[-1]))
        rots = np.zeros((anchor.shape[0], len(rot_names)))
        for idx, attr_name in enumerate(rot_names):
            rots[:, idx] = np.asarray(plydata.elements[0][attr_name]).astype(float)
        ...
        for idx, attr_name in enumerate(anchor_feat_names):
            anchor_feats[:, idx] = np.asarray(plydata.elements[0][attr_name]).astype(float)

        offset_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("f_offset")]
        offset_names = sorted(offset_names, key = lambda x: int(x.split('_')[-1]))
        offsets = np.zeros((anchor.shape[0], len(offset_names)))
        for idx, attr_name in enumerate(offset_names):
            offsets[:, idx] = np.asarray(plydata.elements[0][attr_name]).astype(float)
