...
def gen_image_variants(args, image_list, iteration):
    pipe = AutoPipelineForImage2Image.from_pretrained(args.img2img_model_path, torch_dtype=torch.float16, variant="fp16", cache_dir="/root/autodl-tmp")
    pipe.to(args.sd_device)
    if args.img2img_lora_path is not None:
    ...
    for i in range(0, all_len, args.boot_batch):
        ...
        new_images = pipe(prompts, image=images, num_inference_steps=infer_one_step, 
                        strength=strength, guidance_scale=args.guidance_scale).images
